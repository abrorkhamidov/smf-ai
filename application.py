...
def register():
    ...
    # Checking if user provided all the required fields correctly and if the username is taken
        ...
        if not db.execute("SELECT username FROM users WHERE username = :username",
            ...
            users = db.execute("INSERT INTO users (username, telegram_account, email, hashword) VALUES(:username, :telegram_account, :email, :hash)",
                               username=request.form.get("username"), telegram_account=request.form.get("telegram_account"), email=request.form.get("email"), hash=hashword)
...
def personality():
    ...
    if request.method == "POST":
        extraversion = int(request.form.get("EXT1")) + int(request.form.get("EXT3")) + int(
            request.form.get("EXT5")) + int(request.form.get("EXT7")) + int(request.form.get(
            "EXT9")) - int(request.form.get("EXT2")) - int(request.form.get("EXT4")) - int(
            request.form.get("EXT6")) - int(request.form.get("EXT8")) - int(request.form.get("EXT10"))
        agreeableness = int(request.form.get("AGR2")) + int(request.form.get("AGR4")) + int(
            request.form.get("AGR6")) + int(request.form.get("AGR8")) + int(request.form.get(
            "AGR9")) + int(request.form.get("AGR10")) - int(request.form.get("AGR1")) - int(
            request.form.get("AGR3")) - int(request.form.get("AGR5")) - int(request.form.get("AGR7"))
        consciousness = int(request.form.get("CSN1")) + int(request.form.get("CSN3")) + int(
            request.form.get("CSN5")) + int(request.form.get("CSN7")) + int(request.form.get(
            "CSN9")) + int(request.form.get("CSN10")) - int(request.form.get("CSN2")) - int(
            request.form.get("CSN4")) - int(request.form.get("CSN6")) - int(request.form.get("CSN8"))
        estability = int(request.form.get("EST2")) + int(request.form.get("EST4")) - int(
            request.form.get("EST1")) - int(request.form.get("EST3")) - int(request.form.get(
            "EST5")) - int(request.form.get("EST6")) - int(request.form.get("EST7")) - int(
            request.form.get("EST8")) - int(request.form.get("EST9")) - int(request.form.get("EST10"))
        openess = int(request.form.get("OPN1")) + int(request.form.get("OPN3")) + int(request.form.get("OPN5")) + int(
            request.form.get("OPN7")) + int(request.form.get(
            "OPN8")) + int(request.form.get("OPN9")) + int(request.form.get("OPN10")) - int(
            request.form.get("OPN2")) - int(request.form.get("OPN4")) - int(request.form.get(
            "OPN6"))
        ...
        if not user:
            db.execute(
                ...
                EXT=extraversion, AGR=agreeableness, CON=consciousness, EST=estability, OPN=openess,
    ...
    if request.method == "POST":
        ...
        if not user:
            db.execute(
                ...
                lf1=lf1, lf2=lf2, lf3=lf3, lf4=lf4, lf5=lf5, lf6=lf6, lf7=lf7, lf8=lf8, lf9=lf9, lf10=lf10, lf11=lf11,
                lf12=lf12, lf13=lf13, lf14=lf14, lf15=lf15,
                user_id=session["user_id"])
            return redirect("/index")
        else:
            db.execute(
...
def match():
    ...
    if request.method == "GET":
        ...
        opn = db.execute("SELECT user_id FROM traits WHERE OPN BETWEEN :value1 AND :value2", value1=openess - 5,
                         value2=openess + 5)
        ...
        name = db.execute("SELECT username,telegram_account, email FROM users WHERE user_id=:user_id", user_id=partner)
